...
void GCApplication::showImage() const
{
    ...
    Mat res(image->rows, image->cols, image->type(), cv::Scalar(0));
    Mat binMask;
    if (!isInitialized)
        image->copyTo(res);
    else {
...
void GCApplication::mouseClick(int event, int x, int y, int flags, void *)
{
    switch (event) {
        case CV_EVENT_LBUTTONDOWN: // Set rect or GC_BGD(GC_FGD) labels
        {
            ...
            if ((isb || isf) && rectState
                == SET) //Set the BGD/FGD(labels).after press the "ALT" key or "SHIFT" key,and have finish drawing the rectangle
...
void display(const cv::Mat& originImage, const cv::Mat& alphaMask){
    std::vector<cv::Mat> ch_img(3),ch_bg(3);
    //分离前景和背景通道，设背景色为黑色
    cv::Mat img;
    originImage.convertTo(img, CV_32FC3, 1.0 / 255.0);
    cv::split(img, ch_img);
    cv::Mat bg = Mat(img.size(), CV_32FC3, cv::Scalar(0, 0, 0));
    cv::split(bg, ch_bg);
    //根据alpha的值计算加上透明度以后的图像
    ch_img[0] = ch_img[0].mul(alphaMask) + ch_bg[0].mul(1.0 - alphaMask);
    ch_img[1] = ch_img[1].mul(alphaMask) + ch_bg[1].mul(1.0 - alphaMask);
    ch_img[2] = ch_img[2].mul(alphaMask) + ch_bg[2].mul(1.0 - alphaMask);
    //合并三通道
    cv::Mat res;
    cv::merge(ch_img, res);
    //显示结果
    imshow("matting result", res);
    cv::waitKey();
}

void GCApplication::matting()
{
    if (image->empty() || winName->empty())
        return;
    ...
    if (isInitialized) {
        cv::Mat trimap = generator.getTrimap(*image, mask);
        cv::Mat foreground;
        cv::Mat alpha;
        gm.globalMatting(*image, trimap, foreground, alpha);
        cv::GaussianBlur(alpha, alpha, cv::Size(3, 3), 0);
        display(foreground, alpha);
    }
}